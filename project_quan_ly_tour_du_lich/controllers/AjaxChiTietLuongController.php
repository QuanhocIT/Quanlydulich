...
class AjaxChiTietLuongController
{
    public function handle(): void
    {
        ...
        foreach ($allLuong as $row) {
            ...
            echo '<tr>';
            echo '<td>' . htmlspecialchars((string)$ten_tour) . '</td>';
            echo '<td>' . htmlspecialchars((string)$ngay_khoi_hanh) . '</td>';
            echo '<td>' . htmlspecialchars((string)$phan_tram_hoa_hong) . '%</td>';
            echo '<td>' . number_format((float)$tien_hoa_hong) . '</td>';
            echo '</tr>';
        }
        echo '</tbody></table></div>';
        echo '<div style="margin-top:18px; text-align:right; color:#ffd700; font-size:18px;">Tổng hoa hồng: <b>' . number_format((float)$totalHoaHong) . '</b></div>';
    }
}
