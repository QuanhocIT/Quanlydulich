...
class CongNoHDV {
    ...
    // Bảng thực tế: id, tour_id, hdv_id, so_tien, loai_cong_no, han_thanh_toan, trang_thai, ghi_chu, created_at, updated_at
    public function create(array $data): bool {
        $sql = "INSERT INTO cong_no_hdv (tour_id, hdv_id, so_tien, loai_cong_no, han_thanh_toan, trang_thai, ghi_chu)
                VALUES (?, ?, ?, ?, ?, ?, ?)";
        $stmt = $this->conn->prepare($sql);
        return $stmt->execute([
            $data['tour_id'],
    ...
    public function getByHDV(int $hdv_id): array {
        $sql = "SELECT " . $this->congNoSelectColumns() . " FROM cong_no_hdv WHERE hdv_id = ? ORDER BY created_at DESC";
        $stmt = $this->conn->prepare($sql);
        $stmt->execute([$hdv_id]);
    ...
    public function getChoDuyet(): array {
        $sql = "SELECT " . $this->congNoSelectColumns('cnh') . ", t.ten_tour, nd.ho_ten as ten_hdv
                FROM cong_no_hdv cnh
                JOIN tour t ON cnh.tour_id = t.tour_id
                JOIN nguoi_dung nd ON cnh.hdv_id = nd.id
                WHERE cnh.trang_thai = 'ChoDuyet'
    ...
    // Duyệt hóa đơn (đánh dấu đã thanh toán)
    public function approve(int $id): bool {
        $sql = "UPDATE cong_no_hdv SET trang_thai = 'DaThanhToan' WHERE id = ?";
        $stmt = $this->conn->prepare($sql);
        return $stmt->execute([$id]);
    }

    // Từ chối / đánh dấu quá hạn với ghi chú lý do
    public function reject(int $id, string $ly_do): bool {
        $sql = "UPDATE cong_no_hdv SET trang_thai = 'QuaHan', ghi_chu = ? WHERE id = ?";
        $stmt = $this->conn->prepare($sql);
    ...
    // Lấy danh sách công nợ quá hạn (dùng created_at thay vì ngay_gui)
    public function getQuaHanList(int $days = 7): array {
        $sql = "SELECT " . $this->congNoSelectColumns('cnh') . ", t.ten_tour, nd.ho_ten as ten_hdv
                FROM cong_no_hdv cnh
                JOIN tour t ON cnh.tour_id = t.tour_id
                JOIN nguoi_dung nd ON cnh.hdv_id = nd.id
                WHERE cnh.trang_thai NOT IN ('DaThanhToan')
                  AND cnh.created_at < DATE_SUB(NOW(), INTERVAL ? DAY)
                ORDER BY cnh.created_at ASC";
        $stmt = $this->conn->prepare($sql);
        $stmt->execute([$days]);
        return $stmt->fetchAll();
    }
